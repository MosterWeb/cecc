...
function ajustarMargenSuperior() {
  var nuevoMargenSuperior = ($(window).scrollTop() > 0) ? '14px' : '15px'; // Condición para el margen
  var nuevoAlturaSuperior = ($(window).scrollTop() > 0) ? '28px' : '50px'; // Condición para la altura del logo

  $('.logo_texto').css('margin-top', nuevoMargenSuperior);
  $('.logo_texto').css('max-height', nuevoAlturaSuperior);
}

// Llamamos a la función inicialmente al cargar la página
ajustarMargenSuperior();

// Luego, escuchamos el evento de desplazamiento del menú
